...
export default function Work() {
  ...
  return (
    <section id="work" className="gsap-section py-[72px] border-b border-[var(--ink)]">
      <div className="max-w-[1080px] mx-auto px-8">
        <div className="flex items-baseline gap-4 mb-[44px] overflow-hidden">
          ...
          <h2 className="gsap-header-title font-['Archivo'] font-extrabold text-[clamp(24px,4vw,34px)] tracking-[-0.02em]">Work/Projects</h2>
        </div>

        <div className="gsap-stagger-group grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-7 items-start">
          {works.map((work, index) => (
            <button
              key={work.id}
              data-card
              onClick={() => openModal(index)}
              className="gsap-stagger-card text-left w-full h-full flex flex-col border-2 border-[var(--true-black)] bg-white cursor-pointer transition-transform hover:-translate-y-1 hover:shadow-[4px_4px_0_var(--accent)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2"
            >
              <div className="flex items-center gap-2 py-2.5 px-[14px] border-b-2 border-[var(--true-black)] font-['JetBrains_Mono'] text-[11px] uppercase tracking-[0.04em]">
              ...
              />
              <div className="p-[14px] border-t-2 border-[var(--true-black)] flex flex-col flex-1">
                ...
                <div className="flex flex-wrap gap-1.5 mt-auto">
                  {work.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[11px] font-['JetBrains_Mono'] uppercase tracking-[0.03em] px-2 py-1 border-[1.5px] border-[var(--true-black)]"
                    >
                      {tag}
                    </span>
                  ))}
      ...
      {active && (
        ...
        >
          <div
            className="bg-[var(--paper)] border-2 border-[var(--true-black)] max-w-[860px] w-full max-h-[90vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between py-3 px-4 border-b-2 border-[var(--true-black)] font-['JetBrains_Mono'] text-[11px] uppercase tracking-[0.04em]">
              ...
              <button
                onClick={closeModal}
                aria-label="Close"
                className="px-2 py-1 border-[1.5px] border-[var(--true-black)] hover:bg-[var(--ink)] hover:text-[var(--paper)] transition-colors"
              >
                ESC ✕
              </button>
            </div>
              ...
              {active.images.length > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center border-[1.5px] border-[var(--true-black)] bg-[var(--paper)] hover:bg-[var(--ink)] hover:text-[var(--paper)] transition-colors"
                  >
                    ←
                  </button>
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center border-[1.5px] border-[var(--true-black)] bg-[var(--paper)] hover:bg-[var(--ink)] hover:text-[var(--paper)] transition-colors"
                  >
                    →
                  </button>
                </>
            ...
            <div className="p-4 border-t-2 border-[var(--true-black)]">
